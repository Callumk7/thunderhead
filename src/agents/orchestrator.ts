...
export function Orchestrator() {
	useModel('openai/gpt-5.6-terra');

	const data = useInitialData<v.InferOutput<typeof initialDataSchema>>();
	const reportChannelId = data?.channelId ?? process.env.DISCORD_REPORT_CHANNEL_ID;
	if (reportChannelId) useTool(postMessage({ channelId: reportChannelId }));

	const destination = data?.channelName ? `#${data.channelName}` : 'the configured report channel';
